...
void kstring_show(FILE * o, kstring_t *ks){
	fprintf(o, "k-string (size %ld ) is: ", ks->l);
	for(int i = 0; i < ks->l; i++)
		fprintf(o, "%c", ks->s[i]);
	fprintf(o, "\n");
}

void kseq_show(FILE * o, kseq_t *ks){
	fprintf(o, "[name] "); 		kstring_show(o, &(ks->name));
	fprintf(o, "[comment] ");	kstring_show(o, &(ks->comment));
	fprintf(o, "[seq] ");		kstring_show(o, &(ks->seq));
	fprintf(o, "[qual] ");		kstring_show(o, &(ks->qual));
}
